...
public class BibliController {
    ...
    public String logout(HttpSession session) {
     ...
     @PostMapping("/admin/toggle-adherent/{id}")
    public String toggleAdherentStatus(@PathVariable Integer id, HttpSession session) {
        if (session.getAttribute("adminConnecte") == null) {
            return "redirect:/login-admin";
        }
        
        adherentService.toggleAdherentStatus(id);
        return "redirect:/admin/dashboard";
    }
